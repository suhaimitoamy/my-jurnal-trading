...
const dom = {
  ...
  githubSettingsBtn: document.querySelector("#githubSettingsBtn"),
  githubDialog: document.querySelector("#githubDialog"),
  closeGithubBtn: document.querySelector("#closeGithubBtn"),
  ...
  views: {
    ...
    academy: document.querySelector("#academyView"),
...
function bindEvents() {
  ...
  dom.githubSettingsBtn?.addEventListener("click", () => {
    dom.githubTokenInput.value = state.githubToken;
    dom.githubRepoInput.value = state.githubRepo;
    dom.githubDialog.showModal();
  });
  dom.closeGithubBtn?.addEventListener("click", () => dom.githubDialog.close());
  dom.githubForm?.addEventListener("submit", (e) => {
    e.preventDefault();
    state.githubToken = dom.githubTokenInput.value.trim();
    state.githubRepo = dom.githubRepoInput.value.trim();
    localStorage.setItem(GITHUB_CONFIG_KEY, JSON.stringify({
      token: state.githubToken,
      repo: state.githubRepo
    }));
    dom.githubDialog.close();
    showToast("Pengaturan GitHub disimpan");
...
function loadSettings() {
  try {
    ...
    const githubSaved = JSON.parse(localStorage.getItem(GITHUB_CONFIG_KEY) || "{}");
    state.githubToken = githubSaved.token || "";
    state.githubRepo = githubSaved.repo || "";
...
function setView(view, shouldRender = true) {
  state.view = ["library", "code", "media", "journal", "assistant", "statistics", "notes", "academy"].includes(view) ? view : "library";
...
function renderActiveView({ allMatches, codeItems, mediaItems }) {
  switch (state.view) {
    ...
    case "statistics":
      ...
      break;
    case "academy":
      // Iframe automatically handles the rendering
      break;
...
async function pushJournalToGitHub(journal) {
  if (!state.githubToken || !state.githubRepo) return;
  const path = `jurnal/${journal.date}-${journal.id}.md`;
  const content = `# Jurnal Trading: ${journal.title} (${journal.date})
...
<!-- META: ${JSON.stringify(journal)} -->`;
  
  try {
    let sha = null;
    try {
      const getRes = await fetch(`https://api.github.com/repos/${state.githubRepo}/contents/${path}`, {
    ...
    const body = {
      message: `Simpan jurnal ${journal.date} - ${journal.market}`,
      content: btoa(unescape(encodeURIComponent(content)))
    };
    if (sha) body.sha = sha;

    await fetch(`https://api.github.com/repos/${state.githubRepo}/contents/${path}`, {
      method: "PUT",
      headers: {
    ...
    console.error("Gagal push jurnal ke GitHub", error);
  }
}

async function syncGitHubJournals() {
  if (!state.githubToken || !state.githubRepo) return;
  try {
    showToast("Sinkronisasi jurnal dari GitHub...");
    const res = await fetch(`https://api.github.com/repos/${state.githubRepo}/contents/jurnal`, {
      headers: { "Authorization": `token ${state.githubToken}` }
    });
    if (!res.ok) return;
    ...
    for (const file of files) {
      if (file.name.endsWith('.md')) {
        ...
        const metaMatch = mdText.match(/<!-- META: (.*?) -->/);
        if (metaMatch) {
          try {
            const parsed = JSON.parse(metaMatch[1]);
            const existingIndex = state.journals.findIndex(j => j.id === parsed.id);
            if (existingIndex === -1) {
              state.journals.push(parsed);
              hasNew = true;
            } else {
              // Update if changed (optional)
            }
          } catch(e) {}
        }
      }
    ...
    if (hasNew) {
      saveJournals();
      renderJournals();
      showToast("Jurnal berhasil disinkronkan");
    } else {
      showToast("Sinkronisasi selesai, tidak ada jurnal baru");
    }
  } catch (e) {
    console.error(e);
  }
...
async function saveJournalForm(event) {
  ...
  try {
    ...
    saveJournals();
    pushJournalToGitHub(journal); // Push to github automatically
    state.pendingJournalFiles = [];
